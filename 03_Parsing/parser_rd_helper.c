 ...
 * @return t_ast_node* Pointer to the newly created AST node, or NULL if allocation fails
 */
t_ast_node *create_ast_node(ast_node_type type)
{
...
/**
 * @brief Expects the current token to be of a specific type, advances if correct, sets error if not
 *
 * @param context The parser context containing the current token
